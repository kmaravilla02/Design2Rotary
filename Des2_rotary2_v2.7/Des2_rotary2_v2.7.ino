...
void loop ()
{
  ...
  if (result) {
    result == DIR_CCW ? row_initial = row_initial - 1, prevRow = row_initial + 1 : row_initial = row_initial + 1, prevRow = row_initial - 1;
    if (row_initial < 1) {
    ...
    if (prevRow < 1) {
      prevRow = 3; // roll over to last item }
    }
    else if (prevRow > 3) {
      prevRow = 1;                 // roll over to first item
    }
    print_arrow();
  } //End if result
